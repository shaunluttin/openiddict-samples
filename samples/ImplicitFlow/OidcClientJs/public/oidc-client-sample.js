...
var settings = {
    ...
    client_id: 'OidcClientJs.OidcClient',
...
var diff_redirect_uri = 'http://localhost:5000/oidc-client-sample-callback.html';
...
function signinDifferentCallback(){
    client.createSigninRequest({ data: { bar: 15 }, redirect_uri: diff_redirect_uri}).then(function(req) {
        log("signin request", req, "<a href='" + req.url + "'>go signin</a>");
        if (followLinks()) {
            window.location = req.url;
        }
